...
async function obtenerProductosAleatorios() {
	...
	for (let i = 0; i < 3; i++) {
		...
		const producto = data[indiceAleatorio];
...
async function mostrarProductosAleatorios() {
	...
	for (const producto of productosAleatorios) {
		...
		<img class="card-img-top img-fluid" src="${producto.img}" alt="${
			producto.nombre
		}">
			...
			item.nombre
				...
				<div class="card-body d-flex flex-column justify-content-center align-items-center">
					...
					<button id="agregarBtn${
						item.id
					}" class="btn btn-secondary bg-gris-oscuro text-resalto border-0">Agregar al Carrito <i class="bi bi-cart"></i></button>
                </div>
        </div>`;
...
function mostrarCarrito() {
	...
	for (let item of carritoCompras) {
		contenido += `
            <tr>
                ...
                <td><button class= "btn btn-success" id="sumarUnidadBtn${
									item.id
								}"><i class=""></i>+</button>
                <button class= "btn btn-danger" id="restarUnidadBtn${
									item.id
								}"><i class=""></i>-</button>
                <button class="btn btn-outline-danger" id="eliminarBtn${
									item.id
								}">Eliminar</button></td>
            </tr>
            `;
	...
	for (let item of carritoCompras) {
		...
		document
			.getElementById(`eliminarBtn${item.id}`)
		...
		document
			.getElementById(`sumarUnidadBtn${item.id}`)
			.addEventListener("click", () => {
		...
		document
			.getElementById(`restarUnidadBtn${item.id}`)
			.addEventListener("click", () => {
				let eliminarUnidad = item.restarUni();
				if (eliminarUnidad < 1) {
					let productoRow = document.getElementById(
						`productoCarrito${item.id}`
					);
					productoRow.remove();
...
function calcularTotal(array) {
	...
	montoTotal == 0
		? (document.getElementById(
				"montoTotal"
		  ).innerHTML = `No hay productos en el carrito`)
		: (document.getElementById(
				"montoTotal"
		  ).innerHTML = `TOTAL: ${formatter.format(montoTotal)}`);
...
function buscarProductos(buscado, array) {
	let busquedaArray = array.filter(
		(item) =>
			item.nombre.toLowerCase().includes(buscado.toLowerCase()) ||
			item.categoria.toLowerCase().includes(buscado.toLowerCase()) ||
			item.genero.toLowerCase().includes(buscado.toLowerCase())
	);
	...
	try {
		...
		for (let item of productos) {
            ...
            <tr>
                ...
                <td>${item.nombre}</td>
                <td>${formatter.format(item.precio)}</td>
                <td><button class="btn btn-outline-danger" id="eliminar${
									item.id
